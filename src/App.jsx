...
const COORD_CACHE = 'yc_coords_v3'
...
export default function App() {
  ...
  const [status, setStatus] = useState('카카오맵 로딩 중...')
  const [showSettings, setShowSettings] = useState(false)
  const [email, setEmail] = useState(() => {
    try { return localStorage.getItem(EMAIL_KEY) || DEFAULT_EMAIL } catch(e) { return DEFAULT_EMAIL }
  })

  const activeStations = useMemo(() => allStations.filter(s => s.type === voteType), [allStations, voteType])
  ...
  // ═══════════════════════════════════════
  // 핵심: 카카오맵 SDK 로드 → 지도 생성 → 좌표 변환
  // 전부 하나의 흐름 안에서 순서대로 실행
  // ═══════════════════════════════════════
  useEffect(() => {
    // 1단계: 스크립트 로딩 대기
    const waitScript = () => new Promise(resolve => {
      const check = () => (window.kakao && window.kakao.maps) ? resolve() : setTimeout(check, 100)
      check()
    })

    // 2단계: kakao.maps.load() 호출
    ...
    const geocodeAll = async () => {
      let cache = {}
      try { cache = JSON.parse(localStorage.getItem(COORD_CACHE) || '{}') } catch(e) {}

      const needGeocode = STATIONS.filter(s => !cache[s.id])
      if (needGeocode.length === 0) {
        // 전부 캐시에 있음
        setAllStations(STATIONS.map(s => ({ ...s, lat: cache[s.id].lat, lng: cache[s.id].lng })))
        setStatus('')
        return
      }

      setStatus(`투표소 주소 변환 중... (${needGeocode.length}개, 최초 1회)`)
      const geocoder = new window.kakao.maps.services.Geocoder()

      const geocodeOne = addr => new Promise(resolve => {
        geocoder.addressSearch(addr, (result, stat) => {
          if (stat === window.kakao.maps.services.Status.OK && result.length > 0) {
      ...
      for (const s of needGeocode) {
        ...
        if (coord) cache[s.id] = coord
      }

      try { localStorage.setItem(COORD_CACHE, JSON.stringify(cache)) } catch(e) {}
  ...
  // 마커+원 그리기
  useEffect(() => {
    if (!mapObj.current || !mapReady || activeStations.length === 0) return
  ...
  useEffect(() => {
    ...
    const d = activeStations.filter(s => s.lat).map(s => ({
  ...
  const goStation = useCallback(s => {
    setTab('map')
    setTimeout(() => {
      if (mapObj.current) { mapObj.current.relayout(); mapObj.current.setCenter(new window.kakao.maps.LatLng(s.lat, s.lng)); mapObj.current.setLevel(3) }
    }, 100)
  }, [])
  ...
  const handleShare = async () => {
    const now = new Date(), dateStr = now.toLocaleString('ko-KR')
    ...
    const sub = encodeURIComponent(title), body = encodeURIComponent(text + '\n\n※ 사진을 첨부해주세요')
  ...
  return (
    <div style={S.wrap}>
      ...
      <div style={{...S.bar, background: status?'#555':isViolation?'#d32f2f':'#2e7d32'}}>
        <span style={S.barTxt}>{status || (gpsError ? `⚠ ${gpsError}` : nearest ? `${isViolation?'⛔ 위반구역':'✅ 안전구역'} — ${nearest.name} ${nearest.distance}m` : 'GPS 위치 탐색 중...')}</span>
      </div>
      <div style={S.main}>
            ...
            <div>{distances.map(s=>(
              <div key={s.id} style={{...S.li, borderLeft: s.distance<=100?'4px solid #d32f2f':`4px solid ${voteType==='early'?'#1565c0':'#2e7d32'}`}}>
                ...
                <div style={S.lr}>
                  ...
                  {s.lat&&<a href={kakaoMapUrl(s.name,s.lat,s.lng)} target="_blank" rel="noopener noreferrer" style={S.ml} onClick={e=>e.stopPropagation()}>🗺 크게보기</a>}
                </div>
              </div>
            ))}{distances.length===0&&<div style={S.le}>{myPos?'데이터 로딩 중...':'GPS 위치 확인 후 표시됩니다'}</div>}</div>
          </div>
        )}
      </div>
      {tab==='map'&&(
